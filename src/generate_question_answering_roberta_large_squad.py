...
Fine-tunes RoBERTa-large for extractive reading comprehension on SQuAD 1.1.
...
Expected performance for RoBERTa-large on SQuAD 1.1 (3 epochs, lr=1e-5):
  EM ≈ 86-87%   F1 ≈ 92-93%

Output: ./squad_qa_roberta_large/
'''
...
# ===========================================================================
# Hyperparameters
# RoBERTa-large (355M params) needs a lower lr and smaller batch for stability.
# ===========================================================================
...
EPOCHS         = 3
LR             = 1e-5  # RoBERTa paper: lower lr for large model (was 1.5e-5)
WARMUP_RATIO   = 0.2   # 20% warmup — large model + big vocab needs longer warmup
N_BEST         = 20
MAX_ANS_LENGTH = 30
...
model     = RobertaForQuestionAnswering.from_pretrained('roberta-large').to(device)
...
print('\nFine-tuning RoBERTa-large on SQuAD 1.1...')
best_f1 = 0.0
...
for epoch in range(EPOCHS):
    ...
    if is_main:
        ...
        if metrics['f1'] > best_f1:
            best_f1 = metrics['f1']
            model_to_eval.save_pretrained('squad_qa_roberta_large')
            tokenizer.save_pretrained('squad_qa_roberta_large')
            print(f'     ✓ New best F1 — model saved to ./squad_qa_roberta_large/')

print(f'\nTraining complete. Best F1: {best_f1:.2f}%')
print('Model saved to ./squad_qa_roberta_large/')
